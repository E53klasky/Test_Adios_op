...
int main() {
  try {
    ...
    auto op =
        adios.DefineOperator("SharedCompressor", "CAESAR",
                             {{"accuracy", "0.1"}, {"model", "CAESAR_V"}});
    adios2::Engine writer =
        writeIO.Open("compressed_output.bp", adios2::Mode::Write);

    std::cout << "\n=== Processing wave_3d variable ===" << std::endl;

    int step = 0;
    while (reader.BeginStep() == adios2::StepStatus::OK && step != 20) {
      adios2::Variable<float> varWaveRead =
          readIO.InquireVariable<float>("wave_3d");
      ...
      // if (shape.size() != 3) {
      //     std::cerr << "ERROR: Expected 3D data, got " << shape.size() << "D"
      //     << std::endl; reader.EndStep(); step++; continue;
      // }

      // if (shape[0] < 8) {
      //     std::cerr << "ERROR: First dimension must be >= 8 for CAESAR, got "
      //     << shape[0] << std::endl; reader.EndStep(); step++; continue;
      // }
      ...
      if (step == 0) {
        auto varWaveWrite = writeIO.DefineVariable<float>(
            "wave_3d", {shape[0], shape[1], shape[2]}, {0, 0, 0},
    ...
    std::cout << "\n Successfully compressed " << step << " timesteps"
              << std::endl;
